...
int main(){
	...
	while(1){
		...
		//
		ptr = strstr(real_input,"loader");
		if(ptr == real_input){
			if(fix_isvalid_input(real_input) == -1){
				printf("\"%s\" is a invalid input. Check 'h[elp]'.\n",real_input);	
			}
		}
		else if(isvalid_input(real_input)== -1){// input stirng 이 invalid 한 형태일경우
			printf("\"%s\" is a invalid input. Check 'h[elp]'.\n",real_input);
			continue;
		}
		...
		else if(strcmp(command, "type")== 0){ // cmd_type
			input_switch = cmd_typeFilename(input_tok,par_num);
			if(input_switch == 1){
				add_history(tmp_history);
			}
			else if(input_switch == -1){
		...
		else if (strcmp(command, "assemble") == 0){// cmd_assemble 
			input_switch = cmd_assembleFilename(input_tok,par_num);
			if(input_switch == 1){
				add_history(tmp_history);
			}
			else if(input_switch == -1){
		...
		else if(strcmp(command, "symbol") == 0){// cmd_symbol
			input_switch = cmd_symbol(input_tok,par_num);
			if(input_switch == 1){
				add_history(tmp_history);
			}
			else if(input_switch == -1){
		...
		else if(strcmp(command, "progaddr") == 0){// cmd_symbol
			input_switch = cmd_progaddr(input_tok,par_num);
			if(input_switch == 1){
				add_history(tmp_history);
			}
			else if(input_switch == -1){
		...
		else if(strcmp(command, "loader") == 0){// cmd_symbol
			input_switch = cmd_loader(input_tok,par_num);
			if(input_switch == 1){
				add_history(tmp_history);
			}
			else if(input_switch == -1){
		...
		else if(strcmp(command, "bp") == 0){// cmd_symbol
			input_switch = cmd_bp(input_tok,par_num);
			if(input_switch == 1){
				add_history(tmp_history);
			}
			else if(input_switch == -1){
		...
		else if(strcmp(command, "run") == 0){// cmd_symbol
			input_switch = cmd_run(input_tok,par_num);
			if(input_switch == 1){
				add_history(tmp_history);
			}
			else if(input_switch == -1){
